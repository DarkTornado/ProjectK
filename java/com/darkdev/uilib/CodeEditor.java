...
public class CodeEditor extends AppCompatEditText {
    ...
    public CodeEditor(Context ctx) {
        ...
        Typeface font = Typeface.createFromAsset(ctx.getAssets(), "fonts/RobotoMono-Regular.ttf");
        setTypeface(font);
        initTextWatcher(ctx);
    }

    private void initTextWatcher(Context ctx){
        final String[] blueData = {"function", "return", "end", "local", "if", "then", "else", "elseif", "for", "while", "do", "break", "continue", "in", "with", "true", "false", "nil", "until", "repeat", "and", "or", "not"};
    ...
    public void codeHighlight(Editable s, String[] blueData, String[] redData) {
        ...
        for (String blue : blueData) {
            int start = 0;
            while (start >= 0) {
                int index = str.indexOf(blue, start);
                int end = index + blue.length();
                if (index >= 0) {
        ...
        for (String red : redData) {
            int start = 0;
            while (start >= 0) {
                int index = str.indexOf(red, start);
                int end = index + 1;
                if (index >= 0) {
    ...
    private static void highlightForLua(Editable s, String str) {
        int start = 0;
        while (start >= 0) {
            ...
            if (index >= 0 && end >= 0) {
                ...
                if (span.length > 0) {
                    if (str.substring(index + 1, end).contains("--[[") && str.substring(index + 1, end).contains("]]")) {
                        for (ForegroundColorSpan fs : span) {
                            s.removeSpan(fs);
                        }
        ...
        if (front) {
            try {
                if (str.charAt(end + 1) == '\n') {
                    ...
                    for (char point : points) {
                        if (str.charAt(end + 1) == point) return true;
                    }
                }
            } catch (Exception e) {
                return true;
